...
module.exports = async (context) => {
  ...
  // TODO: Load issues from json (pick name by env)
  const issues = {};
  ...
  while (true) {
    ...
    for (const respIssue of response.issues) {
      ...
      if (issues.hasOwnProperty(issueId)) {
        if (issues[issueId].updated === respIssue.fields.updated) {
      ...
      const issue = {
        ...
        updated: new Date(respIssue.fields.updated),
